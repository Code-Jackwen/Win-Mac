 ...
 * @description
 * @author: xu1
 * @create: 2020-10-14 18:39
 **/
public class StackTest {

    //push、pop、peek
    //add(也可索引，直接添加)、get(下标、indexOf(元素)
    //contains、empty
    //size实际大小、capacity 现有容量，实际只是用了部分
    //removeAll清空全部
    //elements不怎么用  firstElement获取栈底元素、lastElement获取栈顶元素
    public static void main(String[] args) {
        ...
        System.out.println("stack---syso"+stack);
        ...
        System.out.println("2------------"+stack.get(2));
        stack.add(2, p); //入栈       会把当前元素 以及后边元素往后挪动
        System.out.println(stack.get(2));//Person [name=xiaoming, age=20]
        System.out.println("3------------"+stack.get(3));//
        ...
        Object o = stack.get(2);
        System.out.println("改变之前，下表为2的元素值为："+stack.get(2));//改变之前，下表为2的元素值为：com.nyist.xu.dataStructure.stack.Person@4f023edb
        stack.set(2, "改变下标为2的元素--------------set方法");
