...
//    深度优先搜索（DFS） 左边遍历到底，再往右高出边跳。
//    和
//    广度优先搜索（BFS） Z字形遍历树
//    https://blog.csdn.net/weixin_43790623/article/details/105543560
...
//    二叉排序树 百度百科
    ...
    /**
         满二叉树：
            国内定义：每一个层的结点数都达到最大值，二叉树的层数为K，且结点总数是(2^k) -1 ，则它就是满二叉树。
         ...
         B树：
         （英语：B-tree）
         概括来说是一个一般化的二叉查找树（binary search tree），也是自平衡的树，一个节点可以拥有2个以上的子节点。
        B树适用于读写相对大的数据块的存储系统
     B树中每一个内部节点会包含一定数量的键，键将节点的子树分开。
     例如，如果一个内部节点有3个子节点（子树），那么它就必须有两个键。
     左边子树的所有值都必须小于，中间子树的所有值都必须在之间，右边子树的所有值都必须大于。
