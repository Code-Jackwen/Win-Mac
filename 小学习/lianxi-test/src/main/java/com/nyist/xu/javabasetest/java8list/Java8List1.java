...
class Java8List1 {
    //下篇
//    链接：https://www.jianshu.com/p/e429c517e9cb


//    链接：https://www.jianshu.com/p/e429c517e9cb

//    List 的集合转换为 java.util.stream.Stream 类型的流，并对流进行一系列的中间操作，
//    比如过滤保留部分元素、对元素进行排序、类型转换等；最后再进行一个终端操作，
//    可以把 Stream 转换回集合类型，也可以直接对其中的各个元素进行处理，比如打印、比如计算总数、计算最大值等等
//    很重要的一点是，很多流操作本身就会返回一个流，所以多个操作可以直接连接起来
//    流和迭代器类似，只能迭代一次，用完就会销毁。

//    List list = Arrays.asList("1","2","3","4","");
//    List<String> newList = list.stream().map(System.out::println).sorted().limit().collect(Collectors.toList());

    //累计计数 sum、Integer 都要拆箱成一个原始类型再进行 sum 方法求和
    //Java 8 有良心地引入了数值流 IntStream, DoubleStream, LongStream，
    //这种流中的元素都是原始数据类型，分别是 int，double，long

//    1. 流与数值流的转换
//     流转换为数值流
//
//    mapToInt(T -> int) : return java.util.stream.IntStream
...
//     流转换为数值流
//
//    mapToInt(com.nyist.xu.pojo.T -> int) : return java.util.stream.IntStream
//    mapToDouble(T -> double) : return java.util.stream.DoubleStream
//    mapToLong(T -> long) : return java.util.stream.LongStream
//
//    IntStream intStream = list.stream().mapToInt(Person::getAge);
//
//    当然如果是下面这样便会出错
...
//    我们来看看一些数据源的可分解性情况
//    数据源 	可分解性
//    java.util.ArrayList 极佳
//    java.util.LinkedList 差
//    IntStream.range 	极佳
//    Stream.iterate 	差
//    HashSet 好
//    TreeSet 好
...
//    六. 效率
//
//    最后再来谈谈效率问题，很多人可能听说过有关 Stream 效率低下的问题。其实，对于一些简单的操作，比如单纯的遍历，查找最值等等，
//    java.util.stream.Stream 的性能的确会低于传统的循环或者迭代器实现，甚至会低很多。
//
